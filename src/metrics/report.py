...
class Report:
    ...
    def print_process_summary(self):
        ...
        print(f"{'PID':<5} {'AT':<5} {'BT':<5} {'ST':<5} {'FT':<5} "
              f"{'TAT':<8} {'WT':<8} {'RT':<8}")
        ...
        for p in self.metrics.completed:
            ...
            print(
                f"{p.pid:<5} {p.arrival_time:<5} {p.total_burst:<5} "
    ...
    def print_statistics(self):
        ...
        print(f"CPU Busy Time: {self.metrics.get_total_busy_time()}")
        print(f"CPU Idle Time: {self.metrics.get_total_idle_time()}")
    ...
    def print_analysis(self):
        ...
        if self.metrics.get_cpu_utilization() < 100:
            print("- CPU idle time detected → workload is not fully saturating CPU.")
        else:
            print("- CPU fully utilized → high workload saturation.")
